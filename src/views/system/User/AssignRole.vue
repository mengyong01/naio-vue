    ...
    <SysDialog :title="dialog.title" :width="dialog.width" :height="dialog.height" :visible="dialog.visible"
        ...
        <template v-slot:content>
            ...
            <el-table size='small' :height="tableHeight" :data="roleList.list" border stripe>
                <el-table-column width="50" align="center" label="选择">
                    <template #default="scope">
                        <el-radio v-model="selectRoleId" :label="scope.row.id" @change="getSlectRole(scope.row)">{{ ""
                        }}</el-radio>
                    </template>
                </el-table-column>
            ...
            <el-pagination @size-change="sizeChange" @current-change="currentChange"
                :current-page.sync="param.currentPage" :page-sizes="[4, 20, 40, 80, 100]" :page-size="param.pageSize"
...
const { confirm, show, roleList, param, sizeChange, currentChange, tableHeight, selectRoleId, getSlectRole } = useAssignRole(dialog, onClose, onShow)
